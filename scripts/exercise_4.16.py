...
@dataclass
class Model:
    r: float = 0.1 # scaling transition
    q: float = 0.1 # scaling campaign
    ...
    def update(self) -> None:
        ...
        self.c = c_prev + self.r * max(c_prev - l_prev, 0) * l_prev \
                        ...
                        + self.q * (1 - c_prev) * l_prev \
                        + self.q * (1 - c_prev) * n_prev \
                        - self.q * (1 - l_prev) * c_prev
...
class SimulationWindow:
    """Plots c, l, n through time with sliders for `r`, `q`, `c0`, and `l0`.
    ...
    Q_MIN, Q_MAX, Q_STEP = 0.0, 1.0, 0.01
    ...
    def __init__(self, model: Model, steps: int = 30) -> None:
        ...
        self.q_slider.setMinimum(round(self.Q_MIN / self.Q_STEP))
        self.q_slider.setMaximum(round(self.Q_MAX / self.Q_STEP))
        self.q_slider.setValue(round(self.model.q / self.Q_STEP))
    ...
    def _on_q_changed(self, value: int) -> None:
        self.model.q = value * self.Q_STEP
        self._update_plot()
    ...
    def _update_plot(self) -> None:
        ...
        self.q_label.setText(f"q = {self.model.q:.2f}")
